...
export const DebugInfo: React.FC<DebugInfoProps> = ({
  ...
  const totalEnemyHealth = gameState.enemies.reduce(
    (sum, e) => sum + e.health,
    0
  );
  const totalEnemyMaxHealth = gameState.enemies.reduce(
    (sum, e) => sum + e.maxHealth,
    0
  );
  const totalEnemyReward = gameState.enemies.reduce(
    (sum, e) => sum + e.reward,
    0
  );
  const avgEnemyHealth =
    gameState.enemies.length > 0
      ? (totalEnemyHealth / gameState.enemies.length).toFixed(1)
      : 0;

  // Группировка башен по уровням
  const towersByLevel = gameState.towers.reduce(
    (acc, tower) => {
  ...
  const enemiesByLevel = gameState.enemies.reduce(
    (acc, enemy) => {
  ...
  return (
    <div className="debug-info-container" style={styles.container}>
      <h3 className="debug-info-title" style={styles.mainTitle}>
        🔧 Debug Info
      </h3>
      ...
      {onGameSpeedChange && gameState && (
        <div
          className="debug-info-section debug-info-game-speed"
          style={styles.section}
        >
          <h4 className="debug-info-section-title" style={styles.sectionTitle}>
            ⚡ Скорость игры
          </h4>
          <div
            className="debug-info-slider-container"
          ...
          >
            <input
              ...
              style={styles.slider}
            />
            <div className="debug-info-speed-value" style={styles.speedValue}>
              {gameState.gameSpeed.toFixed(2)}x
            </div>
          </div>
        </div>
      )}
      ...
      <div className="debug-info-columns" style={styles.columnsContainer}>
        ...
        <div
          className="debug-info-column debug-info-left-column"
          style={styles.column}
        >
          {/* Общая информация */}
          <div
            className="debug-info-section debug-info-general"
            style={styles.section}
          >
            <h4
              className="debug-info-section-title"
            ...
            <div className="debug-info-content" style={styles.info}>
              <div className="debug-info-row" style={styles.infoRow}>
                <span>Статус:</span>
                <span className="debug-info-value" style={styles.value}>
          ...
          {/* Информация о башнях */}
          <div
            className="debug-info-section debug-info-towers"
            style={styles.section}
          >
            <h4
              className="debug-info-section-title"
            ...
            <div className="debug-info-content" style={styles.info}>
              ...
              {gameState.selectedTowerLevel && (
                <div className="debug-info-row" style={styles.infoRow}>
                  <span>Выбрана:</span>
                  <span
                    className="debug-info-highlight"
                    style={styles.highlight}
                  >
                    Уровень {gameState.selectedTowerLevel}
                  </span>
                </div>
              )}
            </div>
            {Object.keys(towersByLevel).length > 0 && (
              <div className="debug-info-subsection" style={styles.subsection}>
                <div
                  className="debug-info-subsection-title"
            ...
            {gameState.towers.length > 0 && (
              <div className="debug-info-subsection" style={styles.subsection}>
                ...
                {gameState.towers.slice(0, 3).map((tower, idx) => (
                  <div
                    key={tower.id}
                    className="debug-info-entity-card debug-info-tower-card"
                    style={styles.entityCard}
                  >
                    <div
                      className="debug-info-entity-header"
                      style={styles.entityHeader}
                    >
                      #{idx + 1} [L{tower.level}]
                    </div>
                    <div
                      className="debug-info-entity-details"
                      style={styles.entityDetails}
                    >
                      <div>Урон: {tower.damage}</div>
                      <div>Дальн: {tower.range}</div>
                      <div>СкСтр: {tower.fireRate}/s</div>
                      <div>
                        Поз: ({Math.round(tower.position.x)},{" "}
                        {Math.round(tower.position.y)})
                      </div>
                    </div>
                  </div>
                ))}
                {gameState.towers.length > 3 && (
                  <div className="debug-info-more" style={styles.moreInfo}>
                    ...и ещё {gameState.towers.length - 3}
                  </div>
                )}
              </div>
            )}
          ...
          <div
            className="debug-info-section debug-info-enemies"
            style={styles.section}
          >
            <h4
              className="debug-info-section-title"
              style={styles.sectionTitle}
            >
              👾 Враги ({gameState.enemies.length})
            </h4>
            <div className="debug-info-content" style={styles.info}>
              <div className="debug-info-row" style={styles.infoRow}>
                <span>Общее HP:</span>
                <span className="debug-info-value" style={styles.value}>
                  {Math.ceil(totalEnemyHealth)}
                </span>
              </div>
              <div className="debug-info-row" style={styles.infoRow}>
                <span>Макс HP:</span>
              ...
              <div className="debug-info-subsection" style={styles.subsection}>
                ...
                {gameState.enemies.slice(0, 5).map((enemy, idx) => {
                  ...
                  return (
                    <div
                      key={enemy.id}
                      className="debug-info-entity-card debug-info-enemy-card"
                      style={styles.entityCard}
                    >
                      <div
                        className="debug-info-entity-header"
                      ...
                      >
                        ...
                        <div>
                          Путь: {enemy.pathIndex}/{gameState.path.length - 1}
                        </div>
                        <div>
                          Поз: ({Math.round(enemy.position.x)},{" "}
                ...
                {gameState.enemies.length > 5 && (
                  <div className="debug-info-more" style={styles.moreInfo}>
                    ...и ещё {gameState.enemies.length - 5}
                  </div>
                )}
              </div>
            )}
        ...
        <div
          className="debug-info-column debug-info-right-column"
          style={styles.column}
        >
          {/* Информация о снарядах */}
          <div
            className="debug-info-section debug-info-projectiles"
            style={styles.section}
          >
            <h4
              className="debug-info-section-title"
              style={styles.sectionTitle}
            >
              💥 Снаряды ({gameState.projectiles.length})
            </h4>
            {gameState.projectiles.length > 0 && (
              <div className="debug-info-subsection" style={styles.subsection}>
                {gameState.projectiles.slice(0, 5).map((proj, idx) => (
                  <div
                    key={proj.id}
                    className="debug-info-entity-card debug-info-projectile-card"
                    style={styles.entityCard}
                  >
                    <div
                      className="debug-info-entity-header"
                    ...
                    >
                      ...
                      <div>
                        Поз: ({Math.round(proj.position.x)},{" "}
                        {Math.round(proj.position.y)})
                      </div>
                    </div>
                  </div>
                ))}
                {gameState.projectiles.length > 5 && (
                  <div className="debug-info-more" style={styles.moreInfo}>
                    ...и ещё {gameState.projectiles.length - 5}
                  </div>
                )}
              </div>
            )}
          ...
          {/* Информация о пути */}
          <div
            className="debug-info-section debug-info-path"
            style={styles.section}
          >
            <h4
              className="debug-info-section-title"
