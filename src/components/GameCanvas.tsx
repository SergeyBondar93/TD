...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    gameState.fireProjectiles?.forEach((fireProj) =>
      drawFireProjectile(ctx, fireProj)
    );
    ...
    gameState.iceProjectiles?.forEach((iceProj) =>
      drawIceProjectile(ctx, iceProj)
    );
    ...
    if (DEV_CONFIG.SHOW_COORDINATES) {
      ...
      ctx.fillText(
        `X: ${CANVAS_PADDING + GAME_WIDTH}, Y: ${CANVAS_PADDING}`,
        CANVAS_PADDING + GAME_WIDTH - 100,
        CANVAS_PADDING + 15
      );
      ctx.fillText(
        `X: ${CANVAS_PADDING}, Y: ${CANVAS_PADDING + GAME_HEIGHT}`,
        CANVAS_PADDING + 5,
        CANVAS_PADDING + GAME_HEIGHT - 5
      );
      ctx.fillText(
        `X: ${CANVAS_PADDING + GAME_WIDTH}, Y: ${CANVAS_PADDING + GAME_HEIGHT}`,
        CANVAS_PADDING + GAME_WIDTH - 120,
        CANVAS_PADDING + GAME_HEIGHT - 5
      );
    }
...
function drawEnemy(
  ...
  enemy: Enemy,
  deltaTime: number,
  enemy3DManager: ReturnType<typeof getEnemy3DManager>,
  gameSpeed: number
) {
  ...
  if (isLoaded && enemy.modelConfig) {
    ...
    if (modelCanvas) {
      ...
      console.log(
        "[drawEnemy] Model loaded but render returned null for enemy:",
        enemy.id
      );
  ...
  ctx.fillStyle =
    healthPercent > 0.5 ? "#0f0" : healthPercent > 0.25 ? "#ff0" : "#f00";
  ctx.fillRect(
    healthBarX,
    healthBarY,
...
function drawTower(ctx: CanvasRenderingContext2D, tower: Tower) {
  ...
  if (tower.weaponType === WeaponType.LASER) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      tower.position.x - electricSize / 3,
      tower.position.y - electricSize / 3
    );
  ...
  if (tower.buildTimeRemaining > 0) {
    ...
    const singleUpgradeTime =
      (DEV_CONFIG.DEV_BUILD_TIME || GAME_SETTINGS.BASE_UPGRADE_TIME) * 1000;
    ...
    if (tower.upgradeQueue > 0) {
      ...
      totalTime =
        (DEV_CONFIG.DEV_BUILD_TIME || GAME_SETTINGS.BASE_BUILD_TIME) *
        1000 *
        tower.level;
    }
...
function drawFlameStream(
  ...
  const targetEnemies = enemies.filter((e) =>
    stream.targetEnemyIds.includes(e.id)
  );
