...
describe("GameOver", () => {
  ...
  describe("when game is won", () => {
    ...
    it("should display congratulation message with level", () => {
      ...
      expect(
        screen.getByText(/Поздравляем! Вы прошли уровень 5!/)
      ).toBeInTheDocument();
    });
  ...
  describe("when game is lost", () => {
    ...
    it("should display try again message with level", () => {
      ...
      expect(
        screen.getByText(/Вы проиграли на уровне 3\. Попробуйте ещё раз!/)
      ).toBeInTheDocument();
    });
  ...
  describe("buttons", () => {
    ...
    it("should not call handlers multiple times on single click", async () => {
      ...
      const onMenu = vi.fn();
      render(
        <GameOver {...defaultProps} onRestart={onRestart} onMenu={onMenu} />
      );
